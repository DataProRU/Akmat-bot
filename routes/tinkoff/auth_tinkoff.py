...
@router.get("/tinkoff/")
async def get_login_type(request: Request, background_tasks: BackgroundTasks, token: Optional[str] = Query(default=None),  user: dict = Depends(get_authenticated_user),):
    """
    Вход в тинькофф.
    ...
    try:
        ...
        if detected_type:
            return await next_page(request=request, background_tasks=background_tasks, step=detected_type.value, token=token)
        else:
