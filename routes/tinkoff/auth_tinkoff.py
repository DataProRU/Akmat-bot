...
@router.get("/tinkoff/")
async def get_login_type(request: Request, token: Optional[str] = Query(default=None),  user: dict = Depends(get_authenticated_user),):
    """
    Вход в тинькофф.
    ...
    try:
        ...
        detected_type = await detect_page_type(browser, 8)  # Асинхронное определение типа страницы
...
async def login(request: Request, data: str = Body(...), token: Optional[str] = Query(default=None),  user: dict = Depends(get_authenticated_user),):
    """
    Ввод инфы в инпуты
    """
    if token:
...
async def next_page(request: Request, step: str | None = Query(default=None), db: Session = Depends(get_db), token: Optional[str] = Query(default=None), user: dict = Depends(get_authenticated_user),):
    """
