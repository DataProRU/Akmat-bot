...
async def directory(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("directory/type_of_payments.html", {
        "request": request,
        "type_payments": type_payments,
    })

@router.post("/type_of_payments/add", response_class=HTMLResponse)
async def add_type_payments(request: Request, title: str = Form(...), db: Session = Depends(get_db)):
    token = get_token_from_cookie(request)
    if isinstance(token, RedirectResponse):
        return token
...
async def edit_type_payments(request: Request, id: int, title: str = Form(...), db: Session = Depends(get_db)):
    token = get_token_from_cookie(request)
    if isinstance(token, RedirectResponse):
        return token
